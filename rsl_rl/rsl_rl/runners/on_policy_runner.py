...
class OnPolicyRunner:
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        self.save(
            ...
            save_trajectory_statistics=True,
        )
    ...
    def save(self, path, infos=None, save_trajectory_statistics=False):
        torch.save({
        ...
        if (
            getattr(self.env, "trajectory_statistics_enabled", False)
            and (
                getattr(self.env, "trajectory_prioritization_enabled", False)
                or save_trajectory_statistics
            )
        ):
            self.env.save_trajectory_statistics(self._trajectory_statistics_path(path))

    @staticmethod
    def _trajectory_statistics_path(checkpoint_path):
        checkpoint_name = os.path.basename(checkpoint_path)
        checkpoint_stem, checkpoint_extension = os.path.splitext(checkpoint_name)
        if checkpoint_extension != ".pt" or not checkpoint_stem.startswith("model_"):
            raise ValueError(
                "Trajectory statistics require checkpoint filenames in the form model_<iteration>.pt."
            )
        iteration = checkpoint_stem[len("model_"):]
        return os.path.join(
            os.path.dirname(checkpoint_path),
            f"trajectory_statistics_{iteration}.npz",
        )
    ...
    def load(self, path, load_optimizer=True):
        ...
        if (
            getattr(self.env, "trajectory_statistics_enabled", False)
            and getattr(self.env, "trajectory_prioritization_enabled", False)
        ):
            statistics_path = self._trajectory_statistics_path(path)
            if not os.path.isfile(statistics_path):
                raise FileNotFoundError(
                    f"Missing trajectory statistics file required for resume: {statistics_path}"
                )
            self.env.load_trajectory_statistics(statistics_path)
            self.env.reset()
