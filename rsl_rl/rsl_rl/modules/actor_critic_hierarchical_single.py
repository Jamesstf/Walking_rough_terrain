...
class ActorCriticHierarchicalSingle(nn.Module):
    is_recurrent = False
    def __init__(self,  num_actor_obs,
        ...
        super(ActorCriticHierarchicalSingle, self).__init__()
    ...
    def _get_low_level_policies(self, args, device, kwargs):
        ...
        assert len(skill_dict) == 1, "ActorCriticHierarchicalSingle only supports one low-level policy."
    ...
    def _get_num_output(self, frame_stack=1):
        return self.env_cfg_list[0].env.command_dim * frame_stack
    ...
    def _actor(self, observations):
        command = self.actor(observations)
        low_level_observations = self._replace_observations(observations, command, low_high=self.low_high_list[0])
        actions_mean = self.policy_list[0](low_level_observations)
        return {
